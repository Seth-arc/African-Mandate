...
const METRIC_KEYS: ReadonlyArray<{
  key: keyof import('../../state/types').Metrics
  label: string
  negative?: boolean
}> = [
  { key: 'stability', label: 'Stability' },
  { key: 'insurgency', label: 'Insurgency', negative: true },
  { key: 'civilian_support', label: 'Civilian support' },
  { key: 'global_legitimacy', label: 'Global legitimacy' },
  { key: 'regional_synergy', label: 'Regional synergy' },
]
...
export function MetricsPanel(): ReactNode {
  ...
  if (!state.session) {
    return (
      <div className="sidebar-panel" id="metrics-panel">
  ...
  return (
    <div className="sidebar-panel" id="metrics-panel">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        {METRIC_KEYS.map(({ key, label, negative }) => (
          <MetricBar
            key={key}
            label={label}
            value={m[key]}
